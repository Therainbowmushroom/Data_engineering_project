...
Запуск ETL-процесса осуществляется командами (опции обязательны, иначе выводится ошибка):

```
python ./etl/main.py --extract-only        # запуск только процесса экстракции и первичной валидации
python ./etl/main.py --transform-only      # запуск только процесса трансформации датасета и его сохранение
python ./etl/main.py --load-only           # запуск только процесса вторичной валидации, записи файл в *.parqut и загрузки в БД
python ./etl/main.py --full-process        # запуск всего ETL-процесса
```
Для вывода существующих опций используется команда:
```
python ./etl/main.py --help                # вывод существующих опций
```
